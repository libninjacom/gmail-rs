...
/**You should use this struct via [`GmailClient::settings_send_as_create`].

On request success, this will return a [`SettingsSendAsCreateResponse`].*/
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SettingsSendAsCreateRequest {
...
impl SettingsSendAsCreateRequest {}
impl FluentRequest<'_, SettingsSendAsCreateRequest> {
    ...
    pub fn send_as_email(mut self, send_as_email: &str) -> Self {
        self.params.send_as_email = Some(send_as_email.to_owned());
        self
    }
...
impl<'a> ::std::future::IntoFuture for FluentRequest<'a, SettingsSendAsCreateRequest> {
    type Output = httpclient::InMemoryResult<SettingsSendAsCreateResponse>;
    ...
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            ...
            if let Some(ref unwrapped) = self.params.send_as_email {
                r = r.json(json!({ "sendAsEmail" : unwrapped }));
            }
