...
/**You should use this struct via [`GmailClient::threads_list`].

On request success, this will return a [`ListThreadsResponse`].*/
...
impl<'a> ::std::future::IntoFuture for FluentRequest<'a, ThreadsListRequest> {
    type Output = httpclient::InMemoryResult<ListThreadsResponse>;
    type IntoFuture = ::futures::future::BoxFuture<'a, Self::Output>;
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            let url = &format!(
                "/gmail/v1/users/{user_id}/threads", user_id = self.params.user_id
            );
